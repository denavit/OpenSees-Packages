...
{
  public:
    changManderConcrete01(int tag, double ifcc, double iecc, double iEc,
        double irn_pre, double irn_post, double ift, double iet,
        double irp, double ixp_cr, double ixn_cr);
    changManderConcrete01();
    ~changManderConcrete01();

    const char *getClassType(void) const {return "changManderConcrete01";};
  ...
  private:
    ...
    // Useful Function
    void transitionCurve(double Tstrain, double &Tstress, double &Ttangent,
        double ei, double fi, double Ei,
        double ef, double ff, double Ef, int rule);
